...
코드를 변경한 뒤에는 `npm run build:chromium`을 실행하고 `brave://extensions`에서 확장을 다시 로드해야 합니다.
...
| `EXTENSION_ERROR` | 확장과 Discord 탭을 다시 로드합니다. |
...
아이콘 원본과 크기별 PNG는 `icons/`에 있습니다. Discord DOM 탐색과 인라인 UI는 `content.js`, OpenAI 호환 API 요청은 `background.js`, 설정 화면은 `options/`에 구현되어 있습니다.
